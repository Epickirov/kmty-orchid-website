...
//
// Bindings expected on the Pages project (set once in the dashboard):
...
export async function onRequestPost({ request, env }) {
  ...
  // sanitise: cap count, clamp names, validate hex, coerce stock to boolean
  const seen = {};
  const colors = data.colors.slice(0, 40).map(function (c) {
    ...
    if (seen[key]) return false;           // drop exact duplicates
    seen[key] = 1;
    return true;
  });
